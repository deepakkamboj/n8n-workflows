...
class WorkflowSearch {
  ...
  handleSearch() {
    ...
    this.currentResults = this.searchWorkflows(query, {
      category,
      complexity,
      trigger,
    });
  ...
  searchWorkflows(query, filters = {}) {
    ...
    if (filters.category) {
      results = results.filter(
        (workflow) => workflow.category === filters.category
      );
    }

    if (filters.complexity) {
      results = results.filter(
        (workflow) => workflow.complexity === filters.complexity
      );
    }

    if (filters.trigger) {
      results = results.filter(
        (workflow) => workflow.trigger_type === filters.trigger
      );
    }
  ...
  createWorkflowCard(workflow) {
    ...
    const integrationTags = workflow.integrations
      .slice(0, 3)
      .map(
        (integration) => `<span class="integration-tag">${integration}</span>`
      )
      .join("");

    const moreIntegrations =
      workflow.integrations.length > 3
        ? `<span class="integration-tag">+${
            workflow.integrations.length - 3
          } more</span>`
        : "";
    ...
    card.innerHTML = `
            ...
            <p class="workflow-description">${this.escapeHtml(
            ...
            <div class="workflow-actions">
                ...
                }" class="btn btn-primary" target="_blank" onclick="event.stopPropagation()">
                    <i class="fas fa-download"></i> Download JSON
                </a>
                <button class="btn btn-secondary" onclick="event.stopPropagation(); window.copyWorkflowId('${
                  workflow.filename
                }')">
  ...
  createDetailsModal(workflow) {
    ...
    modalContent.innerHTML = `
            <button onclick="this.parentElement.parentElement.remove()" style="position: absolute; top: 1rem; right: 1rem; background: none; border: none; font-size: 1.5rem; cursor: pointer; color: var(--text-muted);"><i class="fas fa-times"></i></button>

            <h2 style="margin-bottom: 1rem;">${this.escapeHtml(
              workflow.name
            )}</h2>

            <div style="margin-bottom: 1.5rem;">
                <strong>Description:</strong>
                <p style="margin-top: 0.5rem;">${this.escapeHtml(
                  workflow.description
                )}</p>
            </div>
            ...
            <div style="display: flex; gap: 1rem;">
                ...
                <button class="btn btn-secondary" onclick="window.copyWorkflowId('${
                  workflow.filename
                }')">
                    <i class="far fa-copy"></i> Copy Filename
                </button>
            </div>
        `;
  ...
  updateResultsHeader(query, filters) {
    ...
    if (filters.complexity)
      filterDesc.push(`Complexity: ${filters.complexity}`);
